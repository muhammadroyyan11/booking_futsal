						...
						foreach ($cart_data as $cart) { ?>
							<tr>
								<td style="text-align:left">
									<?php echo $cart->nama_lapangan ?>
									<input type="hidden" value="<?php echo $cart->nama_lapangan ?>" id="item">
								</td>
								...
								<td style="text-align:center">
									<input type="number" name="durasi[]" class="durasi" id="durasi" min="1">
								</td>
	...
	<table class="table table-striped table-bordered">
		<tbody>
			<tr>
				...
				<td align="right" id="subtotal_bawah">
			...
			<tr>
				...
				<input type="hidden" id="subDiskon">
			</tr>
			<tr>
				...
				<input type="hidden" id="subgTotal">
			</tr>
		</tbody>
	</table>
		...
		<div class="row">
			<div class="col-lg-12">
				...
				<?php if ($cek_keranjang != NULL) { ?>
					...
					<button name="checkout" type="button" id="pay-button" class="btn btn-success" aria-label="Left Align" title="Checkout">
						<span class="glyphicon glyphicon-shopping-cart" aria-hidden="true"></span> Checkout
					</button>
				<?php } ?>
			</div>
...
<script type="text/javascript">
	$('#cekData').click(function(event) {
		event.preventDefault();
		$(this).attr("disabled", "disabled");

		var subTotal = $("#subTotal").val()
		var susubDiskon = $("#subDiskon").val()
		var totalBayar = $("#subgTotal").val()

		console.log(subTotal);
		console.log(susubDiskon);
	...
	$(function() {
		...
		$(document).on("change keyup", ".durasi", function() {
			...
			if (jam_mulai_el.val() != "") {
				...
				<?php if (($this->session->userdata('usertype') != "1" || $this->session->userdata('usertype') == "2") && $diskon['status'] == '1') {
					echo "var disc_ = '" . $diskon['harga'] . "';"; ?>
				<?php } else {
					echo "var disc_ = '0';";
				} ?>
				...
				document.getElementById("subTotal").value = subtotal_bawah;
				var disc = ((disc_ / 100) * subtotal_bawah);
				$("#diskon").html(numberWithCommas(disc));
				document.getElementById("subDiskon").value = disc;
				var gtotal = (subtotal_bawah - disc);
				$("#grandtotal").html(numberWithCommas(gtotal));
				document.getElementById("subgTotal").value = gtotal;
