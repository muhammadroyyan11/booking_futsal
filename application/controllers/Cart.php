...
class Cart extends CI_Controller
{
	...
	public function checkout()
	{
		...
		$this->db->select('harga, status');
		$this->db->where('id', '1');
		$queryDiskon = $this->db->get('diskon')->row();
		
		if ($queryDiskon->status == "1") {
			// Jika status diskon aktif
			$diskonHarga = $queryDiskon->harga;
			$diskon = ($diskonHarga / 100) * $query->total;
			...
			$this->db->update('transaksi', array(
				...
				'diskon'      => $diskon,
				'grand_total' => $query->total - $diskon,
				'deadline'    => date('Y-m-d H:i:s', strtotime('1 hour')),
				'catatan'     => $this->input->post('catatan'),
				'status'      => '1',
			));
		} else {
			// Jika status diskon tidak aktif
			$this->db->where('id_trans', $this->input->post('id_trans'));
			$this->db->where('user_id', $this->session->userdata('user_id'));
			$this->db->update('transaksi', array(
				'subtotal'    => $query->total,
	...
	public function getJamMulai()
	{
		...
		$id_jam = $sekarang->format('H'); // Format jam:menit:detik
		$list_jam = $this->Jam_model->get($id_jam,$tanggal);
