...
class Transaksi extends CI_Controller
{
  public function __construct()
  {
    ...
    if(!$this->ion_auth->logged_in()){redirect('kasir/auth/login', 'refresh');}
		elseif(!$this->ion_auth->is_superadmin() && !$this->ion_auth->is_admin() && !$this->ion_auth->is_kasir()){redirect(base_url());}
  }
  ...
  public function index()
  {
    ...
    $this->load->view('kasir/transaksi/transaksi_list',$this->data);
  }
  ...
  public function create_manual()
  {
    ...
    $this->data['ambil_lapangan'] = $this->Lapangan_model->ambil_lapangan();

    $this->data['lapangan'] = array(
      'name'  => 'lapangan',
      'id'    => 'lapangan',
      'type'  => 'text',
      'class' => 'form-control',
      'value' => $this->form_validation->set_value('lapangan'),
      'required'    => '',
    );
    ...
    $this->data['durasi'] = array(
      'name'  => 'durasi',
      'id'    => 'durasi',
      'type'  => 'number',
      'class' => 'form-control',
      'value' => $this->form_validation->set_value('durasi'),
      'required'    => '',
    );

    $this->load->view('kasir/transaksi/manual_transaksi_add', $this->data);
  }
  ...
  public function create()
  {
    ...
    $this->load->view('kasir/transaksi/transaksi_add', $this->data);
  }
  ...
  public function detail($id)
  {
    ...
    if($row)
    {
      ...
      $this->load->view('kasir/transaksi/transaksi_detail', $this->data);
    }
      else
      {
  ...
  public function print($id)
  {
    ...
    elseif($this->ion_auth->is_admin() && $this->session->userdata('cabang') != $row->cabang)
    {
      ...
      else
      {
        ...
        if($data['cek_data'])
        {
          $this->load->view('kasir/transaksi/print', $data);
        }
          else
          {
            $this->session->set_flashdata('message', "<script>alert('Data tidak ditemukan');</script>");
  ...
  {
    $this->db->select('id, harga,status');
    $this->db->where('id', '1');
    ...
    if ($row)
    {
      ...
      $this->data['action']         = site_url('kasir/transaksi/update_diskon_action');
      $this->data['button_submit']  = 'Simpan';
      $this->data['button_reset']   = 'Reset';
      ...
      // Mendapatkan status yang saat ini tersimpan dalam database (misalnya dari objek $diskon)
      $this->db->select('status');
      $this->db->where('id', '1');
      $query = $this->db->get('diskon')->row();
      $default_status = $query->status;
      
      // Membuat dropdown dengan nilai default
      $this->data['status_dropdown'] = form_dropdown('status', $this->status_options, $default_status, 'class="form-control"');

      $this->load->view('kasir/transaksi/update_diskon', $this->data);
    }
      else
      {
        $this->session->set_flashdata('message', '<div class="alert alert-warning alert">Data tidak ditemukan</div>');
        redirect(site_url('kasir/transaksi/update_diskon/1'));
      }
  }
