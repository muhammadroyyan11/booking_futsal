...
class Auth extends CI_Controller
{
	...
	public function index()
	{
		...
		if (!$this->ion_auth->logged_in()) {
			redirect('kasir/auth/login', 'refresh');
		}
		// Cek super bukan
		elseif (!$this->ion_auth->is_kasir()) {
			...
			$this->_render_page('kasir/auth/index', $this->data);
		}
	}
	...
	public function login()
	{
		...
		if ($this->form_validation->run() == FALSE) {
			...
			$this->_render_page('kasir/auth/login', $this->data);
		} else {
			...
			if ($this->ion_auth->login($this->input->post('identity'), $this->input->post('password'), $remember)) {
				...
				//redirect them kasir to the home page
				...
				// redirect them kasir to the login page
				$this->session->set_flashdata('message', $this->ion_auth->errors(''));
				redirect('kasir/auth/login', 'refresh'); // use redirects instead of loading views for compatibility with MY_Controller libraries
			}
		}
	}
	...
	public function change_password()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			// render
			$this->_render_page('kasir/auth/change_password', $this->data);
		} else {
			...
			if ($change) {
				...
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect('kasir/auth/change_password', 'refresh');
			}
		}
	}
	...
	public function forgot_password()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			$this->data['message'] = (validation_errors()) ? validation_errors() : $this->session->flashdata('message');
			$this->_render_page('kasir/auth/forgot_password', $this->data);
		} else {
	...
	public function reset_password($code = NULL)
	{
		...
		if ($user) {
			...
			if ($this->form_validation->run() === FALSE) {
				...
				// render
				$this->_render_page('kasir/auth/reset_password', $this->data);
			} else {
				...
				if ($this->_valid_csrf_nonce() === FALSE || $user->id != $this->input->post('user_id')) {
					...
					if ($change) {
						if ($user->usertype == '1' or $user->usertype == '2' or $user->usertype == '5') {
							// if the password was successfully changed
							$this->session->set_flashdata('message', $this->ion_auth->messages());
							redirect("kasir/auth/login", 'refresh');
						}
						...
						$this->session->set_flashdata('message', $this->ion_auth->errors());
						redirect('kasir/auth/reset_password/' . $code, 'refresh');
					}
				}
			}
		} else {
			// if the code is invalid then send them kasir to the forgot password page
			$this->session->set_flashdata('message', $this->ion_auth->errors());
	...
	public function deactivate($id = NULL)
	{
		...
		if ($this->ion_auth->logged_in() && $this->ion_auth->is_kasir()) {
	...
	public function create_user()
	{
		...
		if ($this->form_validation->run() == true) {
			...
			if ($_FILES['photo']['error'] <> 4) {
				...
				//file berhasil diupload -> lanjutkan ke query INSERT
					...
					// check to see if we are creating the user | redirect them kasir to the kasir page
		...
		$this->load->view('kasir/auth/create_user', $this->data);
	}
	...
	public function edit_user($id)
	{
		...
		if (!$this->ion_auth->logged_in() || (!$this->ion_auth->is_kasir() && !($this->ion_auth->user()->row()->id == $id))) {
			redirect('kasir/transaksi', 'refresh');
		}
		...
		$this->_render_page('kasir/auth/edit_user', $this->data);
	}
	...
	public function profil()
	{
		...
		$this->load->view('kasir/auth/profil', $this->data);
	}
	...
	public function create_group()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			if ($new_group_id) {
				...
				// redirect them kasir to the kasir page
				$this->session->set_flashdata('message', $this->ion_auth->messages());
			...
			$this->_render_page('kasir/auth/create_group', $this->data);
		}
	}
	...
	public function edit_group($id)
	{
		...
		$this->_render_page('kasir/auth/edit_group', $this->data);
	}
