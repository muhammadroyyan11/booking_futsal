	...
	public function change_password()
	{
		...
		if ($this->form_validation->run() === FALSE) {
			...
			if ($change) {
				...
				$this->session->set_flashdata('message', $this->ion_auth->errors());
				redirect('superadmin/auth/change_password', 'refresh');
			}
		}
	}
			...
			if ($this->form_validation->run() === FALSE) {
				...
				if ($this->_valid_csrf_nonce() === FALSE || $user->id != $this->input->post('user_id')) {
					...
					if ($change) {
						...
						$this->session->set_flashdata('message', $this->ion_auth->errors());
						redirect('superadmin/auth/reset_password/' . $code, 'refresh');
					}
				}
			}
		} else {
		...
		if ($this->form_validation->run() == true) {
			...
			if ($_FILES['photo']['error'] <> 4) {
				...
				//file berhasil diupload -> lanjutkan ke query INSERT
					...
					// check to see if we are creating the user | redirect them back to the superadmin page
						...
						//file berhasil diupload -> lanjutkan ke query INSERT
							...
							if ($this->ion_auth->update($user->id, $data)) {
 					        ...
 					        <div class="alert alert-block alert-success"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>
								...
								redirect(site_url('superadmin/auth/'));
							} else {
								// Set pesan
								$this->session->set_flashdata('message', '
 					        <div class="alert alert-block alert-danger"><button type="button" class="close" data-dismiss="alert"><i class="ace-icon fa fa-times"></i></button>
	...
	public function create_group()
	{
		...
		if ($this->form_validation->run() === TRUE) {
			...
			if ($new_group_id) {
				...
				// redirect them back to the superadmin page
				$this->session->set_flashdata('message', $this->ion_auth->messages());
