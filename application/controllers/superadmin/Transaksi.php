...
class Transaksi extends CI_Controller
{
  public function __construct()
  {
    ...
    if(!$this->ion_auth->logged_in()){redirect('superadmin/auth/login', 'refresh');}
      ...
      else
      {
        ...
        for($i=0;$i<$count;$i++)
        {
          $data_detail[$i] = array(
            ...
            'total'           => $this->input->post('grand_total['.$i.']'),
            'created_by'      => $this->session->userdata('username'),
            'created_date'    => date('Y-m-d'),
            'created_time'    => date("h:i:s")
          );
  ...
  public function detail($id)
  {
    ...
    if($row)
    {
      ...
      else
      {
        ...
        redirect(site_url('superadmin/transaksi'));
      }
  }
  ...
  public function update_diskon($id)
  {
    ...
    if ($row)
    {
      ...
      $this->data['action']         = site_url('superadmin/transaksi/update_diskon_action');
      $this->data['button_submit']  = 'Simpan';
      $this->data['button_reset']   = 'Reset';
      ...
      {
        $this->session->set_flashdata('message', '<div class="alert alert-warning alert">Data tidak ditemukan</div>');
        redirect(site_url('superadmin/transaksi/update_diskon/1'));
      }
  }
